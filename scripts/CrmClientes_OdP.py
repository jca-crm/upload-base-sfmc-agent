...
def ts():
    """Timestamp helper formatted as DD/MM/YYYY HH:MM:SS"""
...
base_name = f"CrmClientes_OdP"  # Insere RUN_ID e deixa entre parentesês assim: CrmClientes_OdP_{RUN_ID}
...
WITH FonteTratada AS (
    ...
    WHERE Email IS NOT NULL
          AND CPF IS NOT NULL
          AND FonteDados NOT IN ('Zoox', 'ClubeGiro', 'WeMobi', 'UTP', 'UL')  -- Filtragem precoce excluindo outras fontes de dados
...
-- CTE para seleção de clientes Outlet
ClientesODP AS (
    SELECT
        FonteDados,
        Nome,
...
def run() -> None:
    """Executes the CRM Clientes Outlet ETL, streaming data in chunks and uploading via SFTP."""
    t0 = perf_counter()
    conn = None
    transport = None
    sftp = None
