...
base_name = f"CrmTransacional_WMB"  # Insere RUN_ID e deixa entre parentesês assim: CrmTransacional_WMB_{RUN_ID}
...
FROM (
    ...
    WHERE DataViagem IS NOT NULL
		  ...
		  --AND DataViagem BETWEEN '2022-01-01' AND '2022-12-31'
...
WHERE CanalVenda IN ('UTP WEMOBI', 'WEMOBI')
...
def run() -> None:
    ...
    try:
        ...
        for n_parts, chunk in enumerate(pd.read_sql(sql, conn, chunksize=CHUNK_SIZE), start=1):
            ...
            # espera 3 minutos antes do próximo chunk (DELAY_SECONDS set to 0 by default)
